...
library(here)
rdir <- here('notdemean', 'testtime','shareEMDSC', 'subdm', 'result', trajectory, 'res')
...
# -----
system.time({
  Res <- testpt(expr = expr, cellanno = cellanno, pseudotime = pseudotime, design=design, permuiter=100, EMmaxiter=100, EMitercutoff=1, verbose=F, ncores=2, type='Time', fit.resolution = 1000, test.pattern = 'overall', demean=FALSE)
